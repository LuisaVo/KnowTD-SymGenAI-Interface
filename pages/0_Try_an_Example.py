...
st.markdown(
    ...
    <div class="steps-flow-grid">
        <div class="steps-flow-step"><strong>1. Select</strong><span>Choose a sample problem card.</span></div>
        <div class="steps-flow-step"><strong>2. Open</strong><span>Load it directly into the text input solver.</span></div>
        <div class="steps-flow-step"><strong>3. Solve</strong><span>Convert to YAML and compute the answer.</span></div>
        <div class="steps-flow-step"><strong>4. Inspect</strong><span>Review values, equations, and reasoning graph.</span></div>
    </div>
    """,
    ...
    for i in range(0, len(example_labels), cols_per_row):
        ...
        for col_idx, label in enumerate(example_labels[i:i+cols_per_row]):
            with cols[col_idx]:
                ...
                if st.button(f"Try {label}", type="primary", key=f"btn_{label}", use_container_width=True):
